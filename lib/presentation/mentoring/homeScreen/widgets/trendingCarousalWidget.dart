        ...
        builder: (context, snapshot) {
          if(snapshot.connectionState == ConnectionState.waiting || snapshot.hasError) {
            ...
            if(profileList.trendingProfilesList.isNotEmpty) {
              final profiles = profileList.trendingProfilesList;

              // Build a single card widget reused for both single and carousel cases
              Widget buildCard(profile) {
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Container(
                          ...
                          child: ClipRRect(
                              ...
                              child: CachedNetworkImage(
                                ...
                                height: 80,
                                fit: BoxFit.cover,
                                placeholder: (context, url) => const Center(child: CustomPlaceHolderImage()),
                        ...
                        Text(
                          profile.expertName,
                          style: TextStyle(color: HexColor(secondaryTextColor), fontWeight: FontWeight.w300, fontSize: 12),
                          textAlign: TextAlign.center,
                        ),
                        const SizedBox(height: 4),
                        Text(
                          languageStringification(profile.languages),
                          style: TextStyle(color: HexColor(lightBlue), fontWeight: FontWeight.w300, fontSize: 12),
                          textAlign: TextAlign.center,
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                        ),
                        ...
                        profile.sessionType.toLowerCase() == "group"
                          ...
                          !profile.availability
                              ? "Request Slot"
                              : profile.session.toLowerCase() == "online" ? "Webinar" : "Seminar",
                          ...
                          children: [
                            Icon(
                              profile.session.toLowerCase() == "online" ? Iconsax.monitor : Iconsax.location,
                            ...
                            Text(
                              sessionClassification(profile.session, profile.sessionType),
                              style: const TextStyle(color: Colors.black, fontWeight: FontWeight.w300, fontSize: 12),
                              textAlign: TextAlign.center,
                            ),
                          ],
              ...
              // Single profile — show centered card, no carousel
              if (profiles.length == 1) {
                return SizedBox(
                  height: 220,
                  child: Center(child: buildCard(profiles.first)),
                );
              }

              // Multiple profiles — show carousel
              return CarouselSlider(
                  items: profiles.map((profile) => buildCard(profile)).toList(),
                  options: CarouselOptions(
                    height: 220,
                    viewportFraction: 0.8,
                    autoPlay: true,
